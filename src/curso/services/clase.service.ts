...
export class ClaseService {
    ...
    async getStateDateSurvey(pfk_estudiante: number, pfk_grupo: number) {
        try {
            ...
            const state = await this.claseRepo.find({ select: ["fecha", "estado_encuesta"], where: { estudiante: pfk_estudiante, grupo: pfk_grupo } });
            ...
            else if (state[0].estado_encuesta === 'Deshabilitado')
                return false
            else if (state[0].fecha == null)
                return false
            else
                return true;
        } catch (error) {
