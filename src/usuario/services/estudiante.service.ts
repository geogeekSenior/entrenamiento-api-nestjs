...
export class EstudianteService {
  ...
  async create(data: CreateEstudianteDto) {
    try {
      const pk_estudiante = await this.estudianteRepo.find({
        select: ['pk_estudiante'],
   ...
   */
  async update(pk_estudiante: number, changes: UpdateEstudianteDto) {
    try {
      const estudiante = await this.estudianteRepo.findOne({
        where: { pk_estudiante: pk_estudiante },
      });
      if (!estudiante) {
        throw new NotFoundException(
